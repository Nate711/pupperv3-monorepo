...
# Local customization: saved-network priority management (see netpriority.py).
# Import works whether comitupweb is loaded as the "web" package (production,
# via comitup-web) or run directly from this directory.
try:
    from web import netpriority  # noqa
except ImportError:
    import netpriority  # noqa
...
def create_app(log):
    ...
    @app.route("/known")
    def known():
        networks = netpriority.list_saved_wifi()
        log.info("known.html - {0} saved networks".format(len(networks)))
        return render_template(
            "known.html",
            networks=networks,
    ...
    @app.route("/known/save", methods=["POST"])
    def known_save():
        order = [u for u in request.form.get("order", "").split(",") if u]
        applied = netpriority.set_priority_order(order)
        log.info("known/save - applied {0} priorities".format(len(applied)))
        return redirect("/known?saved=1")
