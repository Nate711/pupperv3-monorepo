...
class AnimationController : public controller_interface::ControllerInterface {
 ...
 protected:
  bool load_animation_csv(const std::string& name, const std::string& csv_path);
  ...
  void switch_animation(const std::string& animation_name);

  std::shared_ptr<ParamListener> param_listener_;
  Params params_;

  // Animation data loaded from CSV - map from animation name to keyframes
  std::map<std::string, std::vector<std::array<double, kActionSize>>> animations_;
  ...
  // Subscription for animation selection
  rclcpp::Subscription<std_msgs::msg::String>::SharedPtr animation_select_subscriber_ = nullptr;
  realtime_tools::RealtimeBuffer<std::shared_ptr<std_msgs::msg::String>> rt_animation_select_ptr_;
  
  // Track if we need to switch animations
  bool animation_switch_requested_ = false;
  rclcpp::Time animation_switch_time_;
