...
class MockCameraNode(Node):
    def __init__(self):
        ...
        self.get_logger().info(f'Publishing to /camera/image_raw and /camera/image_raw/compressed (quality={self.jpeg_quality})')
    ...
    def load_images(self):
        ...
        if not image_files:
            ...
            # Add text
            cv2.putText(test_image, 'MOCK CAMERA', (width//2 - 100, height//2), 
                       cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
            cv2.putText(test_image, 'No images found', (width//2 - 80, height//2 + 30), 
    ...
    def cycle_image_callback(self):
        ...
        if len(self.images) > 1:
            ...
            self.get_logger().info(f'Switched to image {self.current_image_index + 1}/{len(self.images)}: {image_name}')
