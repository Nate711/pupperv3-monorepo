...
def generate_launch_description():
    ...
    image_folder_arg = DeclareLaunchArgument(
        ...
        description='Folder containing images to cycle through'
    )
    
    cycle_images_arg = DeclareLaunchArgument(
        'cycle_images',
        default_value='True',
        description='Whether to cycle through images'
    )
    
    cycle_interval_arg = DeclareLaunchArgument(
        'cycle_interval',
        default_value='1.0',
        description='Seconds between image changes when cycling'
    )
    ...
    frame_id_arg = DeclareLaunchArgument(
        ...
        description='Frame ID for the camera image'
    )
